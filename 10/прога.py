# Определяем функцию для чтения матрицы из файла
...
# Определяем функцию для записи матрицы в файл
def write_matrix(file_name, matrix):
    with open(file_name, 'w') as file:
        for row in matrix:
            file.write(' '.join(map(str, row)) + 'n')

# Определяем путь к файлу ввода и файлу вывода
...
# Считаем матрицу из файла ввода
matrix = read_matrix(input_file_name)

# Выполняем операции над матрицей (например, вычислите сумму элементов каждой строки)
